...
ROOT_DIR = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/my_MIMIC_IV/patients"
...
def sort_ed(data_list):
    ...
    def custom_sort(item):
        ...
        if item['file_name'] in {"triage", "medrecon", "pyxis", "vitalsign"} and 'charttime' in item:
    ...
    for element in no_time_elements:
        ...
        for i, item in enumerate(time_sorted):
            if item.get("hadm_id") == hadm_id:
                # 插入到同 hadm_id 的最后一个位置
                j = i
                # while j < len(time_sorted) and time_sorted[j].get("hadm_id") == hadm_id:
                #     j += 1
                time_sorted.insert(j, element)
                inserted = True
                break
        if not inserted:
...
if __name__ == "__main__":
    ...
    # rank_per_patient("/mnt/petrelfs/wuchaoyi/my_MIMIC_IV_2.2/case_sample_evaluate/18638427")
