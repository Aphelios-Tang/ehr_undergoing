...
class MIMIC_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if not dict:
            ...
            leng = dict["note"]
        return leng, subject_id, hadm_id
...
if __name__ == "__main__":
    ...
    for i in tqdm.tqdm(range(len(dataset.samples))): 
        leng, subject_id, hamd_id = dataset[i]
        if leng / 4 > 8000:
            long.append((leng, subject_id, hamd_id))
        
        
    # 保存这个字典
    with open("long_history.json", "w") as f:
        json.dump(long, f, indent=4)
