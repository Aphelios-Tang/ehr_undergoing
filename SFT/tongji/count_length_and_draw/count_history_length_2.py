...
class MIMIC_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        total_length = self.process_cases(patient_trajectory_list, subject_id, hadm_id)
        return total_length
    ...
    def process_cases(self, patient_trajectory_list, subject_id, hadm_id, random_flag=True):
        ...
        for ii, item in enumerate(patient_trajectory_list):
            file_name = item["file_name"]
            if file_name == "note" and "DS" in item["items"][0]["note_type"]:
                history_text = get_history(item["items"][0]["text"])
                addention = get_patients_addention(item["items"][0]["text"])
                history_len = len(history_text)
                addention_len = len(addention)
                total_len += history_len + addention_len
        
            elif file_name =="patients":
                item_patients = self.preprocess_func[file_name](item)
                patients_len = len(item_patients)
                total_len += patients_len
...
if __name__ == "__main__":
    ...
    # 把len_list变成dict
    len_dict = {"len_list": len_list}
    
    with open("history_new_len.json", "w") as f:
