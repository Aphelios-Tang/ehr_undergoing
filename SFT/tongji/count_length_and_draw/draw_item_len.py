...
def plot_distribution(key_name):
    # 读取数据
    with open('item_length.json', 'r') as f:
        data = json.load(f)
    ...
    # 设置图表
    plt.title('history len distribution')
    plt.xlabel('len of history (token)')
    plt.ylabel('Density')
    plt.legend()
    plt.grid(True, alpha=0.2)
    
    # 保存
    plt.savefig("fig_history_len/" + key_name + "_len_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()

with open('history_length.json', 'r') as f:
    data = json.load(f)
