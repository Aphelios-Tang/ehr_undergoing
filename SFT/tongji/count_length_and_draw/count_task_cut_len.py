...
class MIMIC_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        free_text, target_task_v = self.process_cases(patient_trajectory_list, subject_id, hadm_id)
        length = len(free_text["Input"] + free_text["Instruction"])
        return length, target_task_v
    ...
    def process_cases(self, patient_trajectory_list, subject_id, hadm_id, random_flag=True):
        ...
        Instruction, history_input_text, transformed_input_text_list, Output = self.task_func[target_task_v](patient_trajectory_list) 
        history = SPLIT_LINE.join(history_input_text) + SPLIT_LINE
        other_len = len(Instruction + history + Output)/4
        while len(SPLIT_LINE.join(transformed_input_text_list) + SPLIT_LINE)/4 + other_len > 8000 and len(transformed_input_text_list) > 0:
            transformed_input_text_list.pop()
        
        transformed_input_text_list = history_input_text + transformed_input_text_list
...
if __name__ == "__main__":
    ...
    task = {
        '1_in_hospital_mortality_prediction': [],
        '2_LOS_prediction': [],
        '3_diagnosis_prediction': [],
        '4_readmission_prediction': [],
        '5_transfer_icu': [],
        '6_medication_combination_prediction': [],
        '7_next_labortory_group': [],
        '8_medical_report_summerization': [],
        '9_treatment_recommendation': [],
        '10_lab_results_interpretation': [],
        '11_service_prediction': []
    }

    for i in tqdm.tqdm(range(len(dataset.samples))): 
        length, task_v = dataset[i]
        task[task_v].append(length)
    
    with open("task_cut_length.json", "w") as f:
