...
class MIMIC_Dataset(Dataset):
    ...
    def get_infer_case(self, idx):
        ...
        free_text, _ = self.process_cases(patient_trajectory_list, subject_id, hadm_id, random_flag=False)
    ...
    def get_item_by_subject_hadm(self, subject_id, hadm_id, random_flag=True):
        ...
        free_text, _ = self.process_cases(patient_trajectory_list, subject_id, hadm_id, random_flag=random_flag)
    ...
    def process_cases(self, patient_trajectory_list, subject_id, hadm_id, random_flag=True):
        ...
        Instruction, transformed_input_text_list, Output = self.task_func[target_task_v](patient_trajectory_list)
...
if __name__ == "__main__":
    ...
    for key in task.keys():
        if len(task[key]) > 0:
            task[key] = sum(task[key]) / len(task[key])
        else:
            task[key] = 0
    print(task)
