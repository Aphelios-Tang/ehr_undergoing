...
class MIMIC_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # )
        # return tokens
        return {"input": free_text["Input"], "instruction": free_text["Instruction"], "output": free_text["Output"], "task_id": task_id}
    ...
    def process_cases(self, patient_trajectory_list, subject_id, hadm_id, task_id="1"):
        ...
        return {
            ...
            "Input": transformed_input_text_list,
...
def create_reconstruction_data(data, tokenizer):
    sample = data["input"]
    # data["input"] is a list
    lenids = 0
    total_output = []
    output = []
    for i in range(len(sample)):
        ...
        i_tokneized = tokenizer(
            i_simple,
            return_tensors="pt",
            # max_length=tokenizer.model_max_length,
            truncation=False,
        )
        i_ids = i_tokneized["input_ids"][0]
        if lenids < 256:
            lenids += len(i_ids)
            output.append(i_simple)
        else:
            total_output.append("\n".join(output)) 
            lenids = 0
            output = []
...
def process_item(idx, dataset, tokenizer):
    sample_out = create_reconstruction_data(dataset[idx], tokenizer)
    lines = []
...
if __name__ == "__main__":
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/mmedlm_model/Qwen2.5-7B-Instruct",
    ...
    output_file = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/ehr_free_text/reconstruction_mini.jsonl"
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for start in range(0, total_samples, batch_size):
            ...
            results = Parallel(n_jobs=n_jobs)(
                delayed(process_item)(i, dataset, tokenizer) for i in batch_indices
            )
            # 将结果展平写入文件
