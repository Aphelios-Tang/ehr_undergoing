...
class MIMIC_Dataset(Dataset):
    ...
    def process_cases(self, patient_trajectory_list, subject_id, hadm_id, task_id="1"):
        ...
        while len(SPLIT_LINE.join(transformed_input_text_list) + SPLIT_LINE)/4 + other_len > 16000 and len(transformed_input_text_list) > 0:
...
if __name__ == "__main__":
    ...
    dataset = MIMIC_Dataset(patient_root_dir = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/my_MIMIC_IV/patients_sorted/",patient_id_csv = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/bohao_mimic_project/supplementary_files/new_split/train_max.csv",tokenizer= tokenizer)
    def process_item(i):
        sample = dataset[i]
        return json.dumps(sample, ensure_ascii=False) + "\n"

    output_file = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/ehr_free_text/test_max_16000.jsonl"
    n_jobs = os.cpu_count() or 1
    batch_size = 100  # 根据内存情况调整批次大小
    total_samples = len(dataset)

    with open(output_file, "w", encoding="utf-8") as f:
        for start in tqdm.tqdm(range(0, total_samples, batch_size)):
            end = min(start + batch_size, total_samples)
            batch_indices = list(range(start, end))
            # 并行处理当前批次
            results = Parallel(n_jobs=n_jobs)(
                delayed(process_item)(i) for i in batch_indices
            )
            # 将当前批次结果写入文件后立即刷新
            for line in results:
                f.write(line)
