...
from number import count_numbers_in_text, NumberPositionEmbedder
...
class Trainer:
    ...
    def process_sample(self, context):
        ...
        model_dtype = self.model.language_model.dtype
        
        # num_net = self.model.get_number_position_embedding(context)
        num_net = self.model.get_number_position_embedding(context).to(device=self.model.language_model.device, dtype=model_dtype)

        # inputs_embeds = torch.cat([soft_prompt, self.model.AE], dim=1)
        inputs_embeds = torch.cat([soft_prompt, num_net, self.model.AE], dim=1)
    ...
    def save(self, epoch, step):
        ...
        if hasattr(self.model, 'A'):
            param['A'] = self.model.A
        if hasattr(self.model, 'B'):
            param['B'] = self.model.B
        if hasattr(self.model, 'C'):
            param['C'] = self.model.C
        if hasattr(self.model, 'D'):
            param['D'] = self.model.D
        if hasattr(self.model, 'number_embedder'):
            param['number_embedder_state_dict'] = self.model.number_embedder.state_dict()
        if hasattr(self.model, 'fusion_gate'):
            param['fusion_gate_state_dict'] = self.model.fusion_gate.state_dict()
...
class ICFormer(BaseModel):
    def __init__(
        ...
        self.ae_num = 32
        self.AE = nn.Parameter(torch.randn([1, self.ae_num, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))
        self.FT = nn.Parameter(torch.randn([1, self.ae_num, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))

        self.num_expand = 8
        self.A = nn.Parameter(torch.randn([1, self.num_expand, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))
        self.B = nn.Parameter(torch.randn([1, self.num_expand, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))
        self.C = nn.Parameter(torch.randn([1, self.num_expand, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))
        self.D = nn.Parameter(torch.randn([1, self.num_expand, icformer.config.hidden_size], device=language_model.device, dtype=language_model.dtype))

        self.number_embedder = NumberPositionEmbedder(hidden_size=icformer.config.hidden_size, expand_ratio=self.num_expand, device_type = language_model.dtype).to('cuda')
        self.fusion_gate = nn.Linear(2 * icformer.config.hidden_size, icformer.config.hidden_size).to('cuda')
    ...
    def get_number_position_embedding(self, text):
        """获取数字位置感知嵌入"""
        # 获取基础类别嵌入
        count, category = count_numbers_in_text(text)
        base_embedding = self.A if category == "A" else self.B if category == "B" else self.C if category == "C" else self.D
        target_device = base_embedding.device
        
        # 获取数字位置嵌入并确保它在正确的设备上
        pos_embedding = self.number_embedder(text).to(target_device)
        
        # 融合两种嵌入
        combined = torch.cat([base_embedding, pos_embedding], dim=-1)
        
        # 确保fusion_gate在正确设备上操作
        fusion_gate = self.fusion_gate.to(target_device)
        gate = torch.sigmoid(fusion_gate(combined))
        
        # 使用门控机制融合两种嵌入
        num_net = gate * base_embedding + (1 - gate) * pos_embedding
    ...
    def train_step(self, step, data):
        ...
        num_net = self.get_number_position_embedding(context).to(device=self.language_model.device, dtype=model_dtype)
        ...
        inputs_embeds = torch.cat([soft_prompt, num_net, self.AE, label_embeds], dim=1)
        # shifted right
