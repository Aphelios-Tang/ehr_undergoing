...
def process_sample(model, tokenizer, context, max_new_tokens=512):
    ...
    model_dtype = model.language_model.dtype
    num_net = model.get_number_position_embedding(context).to(device=model.language_model.device, dtype=model_dtype)
    ...
    inputs_embeds = torch.cat([soft_prompt, num_net, model.AE], dim=1)
...
def evaluate_dataset(model, dataset, num_samples=None, max_new_tokens=512):
    ...
    # 将结果保存到文件
    result_file = "reconstruction_results-expand160000.txt"
...
if __name__ == '__main__':
    ...
    data_dir = "/inspire/ssd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/tangbohao-240108100043/ehr_free_text/reconstruction_256len.jsonl"
    ...
    results = evaluate_dataset(model, data, num_samples=40, max_new_tokens=2048)
